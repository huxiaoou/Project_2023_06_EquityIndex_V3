...
def check_factors(fs: list[str], res_id: str, db_struct: dict, exp_dir: str):
    ...
    sum_df = pd.DataFrame(sum_data).sort_values(["bgn", "factor", "obs"])
...
if __name__ == "__main__":
    check_factors(
        fs=factors,
        res_id="va",
        db_struct=database_structure,
        exp_dir=research_factors_exposure_dir
    )
    check_factors(
        fs=["{}-M{:03d}".format(f, w) for f, w in ittl.product(factors, factor_mov_ave_wins)],
        res_id="ma",
        db_struct=database_structure,
        exp_dir=research_factors_exposure_dir
    )
