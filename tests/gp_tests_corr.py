...
def cal_gp_tests_corr(
    ...
    for factor_lbl in factors:
        ...
        gp_test_lib = CManagerLibReader(t_db_save_dir=os.path.join(tests_result_dir, factor_lbl), t_db_name=gp_test_lib_structure.m_lib_name)
    ...
    plot_corr(t_corr_df=corr_df, t_save_dir=tests_result_summary_dir, t_fig_name="corr-TW{:03d}-{}".format(test_window, uid))
    plot_lines(t_plot_df=ret_cum_df, t_save_dir=tests_result_summary_dir, t_fig_name="cum-TW{:03d}-{}".format(test_window, uid))
